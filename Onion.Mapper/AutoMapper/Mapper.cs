    ...
    protected void Config<TDestination, TSource>(int depth = 5, string? ignore = null)
    {
        ...
        var config = new MapperConfiguration(c =>
        {
            foreach (var item in typePairs)
            {
                ...
                {
                    c.CreateMap(item.SourceType, item.DestinationType).MaxDepth(depth).ForMember(ignore, x => x.Ignore()).ReverseMap();
                }
                else
                {
                    c.CreateMap(item.SourceType, item.DestinationType).MaxDepth(depth).ReverseMap();
                }
            }
        });
